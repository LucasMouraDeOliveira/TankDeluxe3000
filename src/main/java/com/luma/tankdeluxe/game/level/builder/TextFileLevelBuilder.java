...
public class TextFileLevelBuilder {
	...
	// loadWallsAndSpawns(level,
	// Files.readAllLines(levelFolder.resolve("walls.txt")));
	// return level;
	// } catch(IOException e) {
	// return new Level();
	// }
		...
		for (String line : lines) {
			...
			for (char c : line.toCharArray()) {
				if (c == '1') {
					...
					// cell.setWall(new Square(col * SettingsManager.OBSTACLE_WIDTH, row *
	...
	private void addSpawns(Level level, List<String> lines) {
		...
		for (String line : lines) {
			...
			for (char c : line.toCharArray()) {
				if (c == '2') {
					level.addSpawn(new Coordinate((int) (col * SettingsManager.OBSTACLE_WIDTH),
							(int) (row * SettingsManager.OBSTACLE_HEIGHT)));
				}
				col++;
			}
			row++;
