...
import com.luma.tankdeluxe.service.user.PlayerLevelService;
...
public class ScoreService {
	...
	public void addScoreEntryIfEligible(String username, int score) {
		...
		Optional<ScoreEntry> existingEntry = scores.stream().filter(entry -> entry.getUsername().equals(username))
				.findAny();
	...
	private ScoreEntry createScoreEntry(String username, int score) {
		ScoreEntry newEntry = new ScoreEntry();

		newEntry.setUsername(username);
		newEntry.setScore(score);

		return this.scoreEntryRepository.save(newEntry);
	}

	public void increasePlayerXp(String name, int xpAmount) {
		this.playerLevelService.addXp(name, xpAmount);
	}
